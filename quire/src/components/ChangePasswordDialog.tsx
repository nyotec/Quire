...
interface Props {
  open: boolean;
  /** "change" prompts for current+new; "disable" prompts only for current. */
  variant: 'change' | 'disable';
  onCancel: () => void;
  /** Validates current password (resolves true if correct). */
  onVerifyCurrent: (currentPassword: string) => Promise<boolean>;
  /** "change": commit with current+new; "disable": commit with current only. */
  onCommit: (args: { currentPassword: string; newPassword?: string }) => Promise<void>;
}
...
export function ChangePasswordDialog({
  ...
  const submit = async () => {
    if (!canSubmit) return;
    setPhase('working');
    const ok = await onVerifyCurrent(current);
    if (!ok) {
      setPhase('error');
    ...
    <div className="q-modal-scrim">
      <div className="q-modal q-pw-dialog" onClick={(e) => e.stopPropagation()}>
        <div className="q-modal-title">
          {variant === 'change' ? 'Change password' : 'Disable password protection'}
        </div>
        <div className="q-modal-body">
          {variant === 'disable' && (
            <p>
              This decrypts every leaf and writes the wiki back as plaintext. The file
